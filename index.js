/* Aşağıda global olarak tanımlanmış değişkenler bulunmaktadır, bunları değiştirmeyiniz. 
...
/* (Oto test yok) Yukarıdaki KareninAlani fonksiyonunu kenar uzunluğu = 10 vererek aşağıda çalıştırıp, sonucu konsolda gözlemleyin 
(console.log)  */
...
- CemberinAlani fonksiyonunu kullanarak aşağıdaki yönergeleri uygulayın:
	...
	3. Çemberin alanı hesaplanacaktır (💡 İPUCU: Çemberin alanı = pi * yarıçapın karesi, yarıçapın karesini bulmak için Javascript 
  içinde tanımlı Math kütüphanesini kullanabilirsiniz. Math.pow(yaricap,2))
...
/* (Oto test yok) Yukarıdaki CemberinAlani fonksiyonunu yarıçap = 15 vererek aşağıda çalıştırıp, sonucu konsolda gözlemleyin 
(console.log)  */
...
/* 	GÖREV 3:
  ...
  Bu diziyi kullanarak aşağıdakileri uygulayın:
		3a. enbuyuk ve enkucuk isminde 2 adet değişken tanımlayın ve sayilar dizisindeki en küçük sayı ile en büyük sayıyı bu 
    değişkenlere atayın. (for döngüsü kullanın)
    ...
    (.forEach metodunu kullanın)
		...
		3c. `ucetambolunenler` dizisindeki sayıların toplamını .reduce metoduyla bulup, sonucu `ucebolunenlerintoplami` 
    değişkenine yazdırın (.reduce metodunu kullanın)
    ...
    birden fazla kez yazılmış sayıları tespit ederek kaç kere tekrar edildiğini belirten bir string oluşturulup `tekraredensayilar` dizisine aktarılmasını istiyoruz. 
...
ucebolunenlerintoplami = ucetambolunenler.reduce((sayilarinToplami,simdikiSayi)=>{
return sayilarinToplami+simdikiSayi;
},0);
...
siralisayilar=besyuzdenkucuksayilar.sort((enKucuk,enBuyuk)=> enKucuk-enBuyuk);

// 3f çözümü


  tekraredensayilar =[];
  const tekrarEdenSayi= {};
  for (let i=0; i<sayilar.length; i++)
  {
      ...
      {
        tekrarEdenSayi[sayi]=1;
      }
  }
for (const sayiAnahtari in tekrarEdenSayi) {
  const adet = tekrarEdenSayi[sayiAnahtari];

  if (adet > 1) {
    const formatliString = `${sayiAnahtari} sayısı ${adet} kere tekrar edilmiştir`;
    tekraredensayilar.push(formatliString);
  }
}
